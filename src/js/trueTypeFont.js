...
export class TrueTypeFont {
  ...
  drawGlyphCurved(index, ctx) {
    ...
    for (let i = 0; i < glyph.numberOfContours; i++) {
      contours.push(
        glyph.points.slice(
          [glyph.contourEnds[i - 1] + 1 || 0],
          glyph.contourEnds[i] + 1
        )
      );
    }

    const completeContours = contours.map(fillTheGaps);

    for (const contour of completeContours) {
      for (let i = 0; i < contour.length; i++) {
        ...
        if (i === 0) {
          ...
          const b = i === contour.length - 1 ? 0 : ++i;
          ctx.quadraticCurveTo(point.x, point.y, contour[b].x, contour[b].y);
        }
...
function fillTheGaps(points) {
  const newPoints = [];
  let lastPoint = points[points.length - 1];
  for (let i = 0; i < points.length; i++) {
    if (!points[i].onCurve && !lastPoint.onCurve) {
      newPoints.push(calcMidPoint(lastPoint, points[i]));
    }
